...
export class PaymentService {
    ...
    async cancelPayout(payoutId: string) {
        return await this.stripeClient.payouts.cancel(
            'po_1K8Kox2eZvKYlo2CDkhI95ov',
        );
    }
    ...
    async createStripeCustomerAndCard(userId: string, stripeToken: string) {
        try {
            ...
            this.userService.addStripeCustomerId(user._id, stripeCustomerId, stripeCardId)
    ...
    async dashBoardStats(providerId: string) {
        ...
        const balance = await this.getStripeBalance(
            isProviderExist.stripeAccountId,
        );
